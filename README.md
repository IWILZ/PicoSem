...
This library uses 2 semaphores and 2 unidirectional data buffers to reach the goal but it's important to underline that each buffer is "one-way" and is also a "common resource" betwen the 2 cores (one writing and the other reading) and so to make a real bidirectional data exchange, you will need to define 2 different data buffers.
